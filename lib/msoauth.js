...
// リソース識別子は outlook.office.com(365 は付かない)。IMAP の接続先ホスト名
// outlook.office365.com とは別物なので注意。MS_SCOPES で上書きもできる。
const SCOPES =
...
// JWT のペイロードからメールアドレスらしきクレームを取り出す(取れなければ null)
function claimEmail(token) {
  if (typeof token !== 'string') return null;
  const parts = token.split('.');
  if (parts.length < 2) return null;
  try {
    const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'));
    return payload.preferred_username || payload.email || payload.upn || payload.unique_name || null;
  } catch (_) {
    return null;
  }
}
...
async function exchangeCode(req, code) {
  ...
  // メールアドレスは id_token から取る。取れない場合はアクセストークン(JWT のことが多い)も見る。
  const email = claimEmail(data.id_token) || claimEmail(data.access_token);
