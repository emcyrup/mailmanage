...
// 原因が分かりにくいサーバー応答に、対処方法のヒントを添える
const ERROR_HINTS = [
  {
    match: /authenticated but not connected/i,
    hint: 'Outlook.com の設定で IMAP が無効になっている可能性があります。'
      + 'Outlook.com →設定→メール→「転送と IMAP」で「デバイスとアプリで IMAP を使用できるようにする」をオンにしてください。',
  },
  {
    match: /invalid credentials|authentication failed|login failed/i,
    hint: 'パスワードが違うか失効しています。Gmail / iCloud はアプリパスワードを再発行して登録し直してください。',
  },
];

function describeError(err) {
  const raw = err.responseText || err.message || String(err);
  const found = ERROR_HINTS.find((h) => h.match.test(raw));
  return found ? `${raw} — ${found.hint}` : raw;
}
...
async function checkInbox(account) {
  // OAuth アカウントでトークン取得に失敗している場合は接続を試みない
  if (account.tokenError) {
    return { ok: false, error: account.tokenError, checkedAt: new Date().toISOString() };
  }
  const client = new ImapFlow({
    ...
    auth: account.accessToken
      ? { user: account.user, accessToken: account.accessToken }
